...
## v2.4 로그인 안전 패치
- 전 버전과 비교 결과, Google OAuth 설정/버튼 ID/signInWithOAuth 흐름은 유지했습니다.
- 앱 내부 UI 이벤트 초기화보다 Google 로그인 버튼 연결을 먼저 실행하도록 변경했습니다.
- 화면 요소 일부에서 오류가 발생해도 로그인 버튼이 죽지 않도록 초기화 오류를 분리했습니다.
