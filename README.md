...
- 홈 화면의 요약 카드를 PC에서 3열 카드형으로 정리했습니다.
- 주간 보기를 PC에서 7일 가로 카드 형태로 보이도록 수정했습니다.
- 월간 보기의 휴무/연차 색상을 빨간 계열로 통일했습니다.
- 데이터 입력 버튼명을 `검수표에 반영` / `반영 후 저장`으로 정리하고 차이 설명을 추가했습니다.
- 코드 설정 화면을 PC에서 여러 열 카드 그리드로 보이도록 보정했습니다.
- OCR 보조의 예전 임시 이름 목록은 자동으로 현재 팀 기본 이름으로 교체되도록 처리했습니다.
