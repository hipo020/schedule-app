...
## v2.8 데이터 확인 목록 개선
- 데이터 확인 탭의 `확인 필요 항목`에서 더 이상 `외 n개`로 숨기지 않고 전체 문제 목록을 스크롤 영역으로 모두 표시합니다.
- 미등록 코드는 코드별로 묶어서 어느 사람의 몇 일에 들어갔는지 펼쳐서 확인할 수 있습니다.
- 미등록 코드는 코드 설정에 추가하면 검수 목록에서 사라진다는 안내를 추가했습니다.
