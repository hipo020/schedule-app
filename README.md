...
## v2.5 모바일 레이아웃 정리
- 상단 앱명/계정 영역을 모바일에서 더 작고 안정적으로 보이도록 압축했습니다.
- 메인 설명 문구는 사용자에게 필요한 최소 문구로 줄였습니다.
- 모바일 홈/일간/주간/월간/휴무일 화면의 세로 공간을 줄이고 주요 정보가 바로 보이도록 정리했습니다.
- 모바일 주간 보기는 세로 리스트형으로, PC 주간 보기는 가로 카드형으로 유지했습니다.
- 데이터 입력 화면은 모바일에서 1번, 2번 안내 카드가 세로로 순서대로 보이도록 변경했습니다.
